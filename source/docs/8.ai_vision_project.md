...
:::{Note}
If there is no camera returned image, camera is not connected. Please check whether the camera cable is connected to Raspberry Pi board.
:::

(4) Image on right is real-time image transmitted by camera, and image on left displays the collected color. Focus camera on red ball. Then drag six sliders below to make the red ball turn white and other area turn black. After that, click "**Save**" button.
...
:::{Note}
If the transmitted image does not appear in the pop-up window, it indicates the camera is not connected properly. Check the camera connection cable to ensure it is securely connected.
:::

(4) The image on the right side of the interface shows the real-time transmitted video, and the left side shows the color to be captured.
...
(4) check whether the camera cable is connected to Raspberry Pi board.
...
(4) Image on right is real-time image transmitted by camera, and image on left displays the collected color. Focus camera on black line. Then drag six sliders 
...
|     2      |     TurboPi goes backward a certain distance  |
